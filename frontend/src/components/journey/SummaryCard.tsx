  ...
  return (
    <div className="rounded-2xl bg-white/95 text-neutral-900 ring-1 ring-neutral-200 shadow-sm p-4">
      ...
      <div className="space-y-1 text-sm">
        ...
        <div className="flex justify-between"><span>Filtros Premium</span><span>{`USD ${filtersPerPax.toFixed(2)}`}</span></div>
        <div className="flex justify-between"><span>Add-ons</span><span>{`USD ${addonsPerPax.toFixed(2)}`} {cancelCost > 0 && <span className="text-xs text-neutral-500">(incl. Seguro de cancelación)</span>}</span></div>
