...
export default function AnimatedDeckCard({ addon, active, onClick }: Props) {
  ...
  // Detectar entrada en viewport para autotilt
  const inView = useInView({ amount: 0.3, once: true })

  // Autotilt de 1.2s la primera vez que aparece (se saltea si el usuario prefiere menor animación)
  useEffect(() => {
    if (prefersReduced) return
    if (!inView) return
    // pequeña “coreografía” que mueve los motion values y vuelve a 0
    const controls = [
      animate(my, [-0.12, 0.1, 0], { duration: 1.2, ease: 'easeInOut' }),
      animate(mx, [0.14, -0.08, 0], { duration: 1.2, ease: 'easeInOut' }),
    ]
