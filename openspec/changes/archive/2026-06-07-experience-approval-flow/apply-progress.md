...
- [x] 4.5 `src/app/[locale]/(secure)/dashboard/admin/AdminExperiencesPageClient.tsx` — tabs (all/pending), clickable PENDING_REVIEW rows, wired admin review
- [x] 4.6 `src/app/[locale]/(secure)/dashboard/admin/AdminReviewSlot.tsx` (NEW) — Per-type price inputs (XSED filtered), approve/reject actions
...
| D2 | Server strips `status`/`pricingByType`/`reviewNote` from tripper PATCH — no client bypasses |
| D3 | Used tab-based admin review page (not side panel) — full form context visible, shareable URL |
| D4 | Single fetch + client-side filter for Pending Review tab |

## Post-Verify Fix Applied

After verification pass, the following issue (W6) was fixed:
- `allTabsComplete` in `ExperienceFormContent.tsx` was including the synthetic "admin-review" tab, causing `false` when admin viewed other tabs.
- Fix: `NewExperienceShell.tsx` now filters out "admin-review" from `completedTabIds` before passing to `ExperienceFormContent`.
- This ensures `allTabsComplete` computes correctly over only the real tripper tabs.
