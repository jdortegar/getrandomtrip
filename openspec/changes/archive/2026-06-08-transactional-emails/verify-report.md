...
| D4 — Admin PATCH branching on nextStatus | Yes | `if/else if/else if` on `nextStatus` after update in admin route |
| D5 — WelcomeEmail in `!dbUser` branch | Yes | Inside Google OAuth conditional on line 87 of `auth.ts` |
| D6 — ExperienceSubmitted to ADMIN_EMAIL, ES-only | Yes | `to: process.env.ADMIN_EMAIL ?? "hola@getrandomtrip.com"`, hardcoded `locale="es"` |
| D7 — Absolute CTA URLs with BASE_URL | Yes | All templates define `const BASE_URL = "https://getrandomtrip.com"` |
