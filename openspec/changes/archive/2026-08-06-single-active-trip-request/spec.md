# Delta Spec: single-active-trip-request

## Change: `single-active-trip-request`

Introduces a `trip-request-lifecycle` capability (new — `openspec/specs/` has no existing trip-request capability today) formalizing: (1) family-scoped single-active-trip enforcement in `POST /api/trip-requests`, (2) persisted expiry-revert of stale `PENDING_PAYMENT` rows, (3) removal of the dead `POST /api/trips` handler, and (4) the one-off historical-duplicate cleanup script. See `proposal.md` for the 9 locked decisions this spec formalizes.

---

# trip-request-lifecycle Specification (NEW)
