...
- [x] 8.5 Manual QA (tripper) — **not executable in this sandbox.** Regression pass: `DRAFT` → `PENDING_REVIEW`, submitted email fires, approval pipeline intact. (Automated regression coverage for this exists via the unchanged/passing pre-existing submit-route tests — see 8.2.)
- [x] 8.6 Manual QA (reviews) — **not executable in this sandbox.** Automated equivalent covered in Phase 4's new tests (RANDOMTRIP → no attribution, TRIPPER → attribution preserved), but a live token-based submission QA pass is still recommended before merge.
