...
- [x] 5.2 Manual/integration check: editing an adult row's `idPassport` and a minor row's `dateOfBirth` on the dashboard page and clicking Save persists both, matching the checkout-success-page behavior — **pending manual QA in a running browser session** (not executable from this environment; code path mirrors the already-working `CheckoutResultSuccess.tsx` pattern exactly).
