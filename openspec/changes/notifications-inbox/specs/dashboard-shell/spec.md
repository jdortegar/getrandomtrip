...
#### Scenario: Unread dot refreshes after marking a notification unread

- GIVEN the admin's unread dot is showing a count that predates a recent action
- WHEN the admin marks a read notification as unread from the reading pane
- THEN the unread dot count increments to reflect the new unread item, without a page reload, remount, or navigation
