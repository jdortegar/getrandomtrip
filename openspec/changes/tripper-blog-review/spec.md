# Delta Spec: tripper-blog-review

## Change: `tripper-blog-review`

Adds a new `blog-review-flow` capability formalizing the tripper submit → admin moderation → review-copy → publish lifecycle for `BlogPost`, replacing the current unmoderated `handleTogglePublish` direct-toggle. No existing `openspec/specs/` blog spec exists — this is a NEW capability, not a delta against prior behavior. See `proposal.md` for the locked decisions this spec formalizes and the one open question (review-copy terminal state) intentionally left to `design.md`.
