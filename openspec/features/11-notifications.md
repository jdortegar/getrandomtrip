...
Surfaces system events as in-app messages for both travelers and trippers. Trippers have a dedicated inbox and an unread indicator in the nav. Travelers see a sidebar panel on their dashboard. Notifications complement transactional emails for time-sensitive events.

## What's Implemented

- `GET /api/notifications` with audience filter (TRAVELER / TRIPPER)
...
- Traveler `NotificationsPanel` on the dashboard sidebar
- Notifications emitted for: experience approved, experience rejected

## Gaps

- [ ] No TRAVELER unread indicator — no unread-count endpoint for travelers, no UI dot or badge
