# Feature Spec: Traveler Dashboard

**Priority:** 5 — Post-booking traveler experience  
**Routes:** `/dashboard/traveler`, `/dashboard/traveler/trips`, `/dashboard/traveler/reviews`, `/dashboard/traveler/notifications`, `/dashboard/traveler/settings`, `/dashboard/trips/[id]`, `/account`, `/trips/[id]` (redirect shim)  
...
The traveler dashboard lives under `/dashboard/traveler/*` and uses the shared `DashboardRoleShell` — a horizontal tab strip driven by `travelerNav` config. Legacy routes redirect:

- `/dashboard` → `/dashboard/traveler`
- `/dashboard/settings` → `/dashboard/traveler/settings`

### Requirement: Traveler route segment

The traveler dashboard SHALL live under `/dashboard/traveler/*`.

| Tab | Route |
|-----|-------|
| Home | `/dashboard/traveler` |
| My Trips | `/dashboard/traveler/trips` |
| My Reviews | `/dashboard/traveler/reviews` |
| Notifications | `/dashboard/traveler/notifications` |
| Settings | `/dashboard/traveler/settings` |

Trip detail pages remain at `/dashboard/trips/[id]/*` outside the traveler tab shell.

### Requirement: Traveler home content

The traveler home tab SHALL show a summary dashboard:
...
### Requirement: Traveler notifications tab

The notifications tab SHALL list notifications with `audience=TRAVELER`, ordered by `createdAt desc`.

Clicking a notification with `tripRequestId` metadata SHALL navigate to `/dashboard/trips/[id]`.

### Requirement: Traveler settings tab

The settings tab SHALL render `AccountSettingsPanel` with `role="traveler"` inside the tab shell (no `HeaderHero`).
...
- `/dashboard/traveler/*` — any authenticated user with a known role
...
Redirect target SHALL follow priority: admin > tripper > traveler.
...
**Post-booking flow:**
1. Pay → `/checkout/success` → primary CTA links to `/dashboard/traveler` ✅
2. `/dashboard/traveler` → see trip in Upcoming panel → click "Ver detalles" → `/dashboard/trips/[id]`
3. View logistics (origin city, dates, traveler count, transport, filters, addons, pricing)
4. Trip transitions to REVEALED → destination toggle appears → click to reveal → **no traveler notification is sent**
...
| HIGH | Destination reveal does not trigger a traveler notification |
...
| LOW | No empty-state handling when a traveler has no trips in the dashboard |
...
6. **Write traveler notification on REVEALED** — trigger when admin patches trip status to `REVEALED`.
