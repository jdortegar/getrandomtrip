...
(Previously: this endpoint returned the assigned experience's `itinerary`/`inclusions`/`exclusions` unconditionally for any status reachable via `canAccessTrip`, with no status-based gate at all. Gate then narrowed to `{REVEALED, COMPLETED}` in an earlier revision of this spec, then widened to include `CANCELLED` by founder decision: an already-issued voucher may be needed for a refund/cancellation dispute, and a cancelled trip has no future "surprise" left to protect regardless of reveal history.)
