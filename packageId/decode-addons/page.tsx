...
// frontend/src/app/packages/[type]/decode-addons/page.tsx
'use client';
...
const decodeAddons: Addon[] = [
  ...
  { id: 'alquiler-vehiculo', title: 'Alquiler de vehículo en destino', description: 'Precio fijo por reserva', price: 50, costType: 'per_reservation' }, // Assuming per reservation, not per day for simplicity
  { id: 'experiencia-destino', title: 'Experiencia en destino', description: '', price: 50, costType: 'per_person' },
];

const DecodeAddonsPage = () => {
  ...
  const calculateAddonCosts = useMemo(() => {
    ...
    selectedAddons.forEach((addonId) => {
      const addon = decodeAddons.find((a) => a.id === addonId);
      if (addon) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.push(`/packages/${travelType}/${packageId}/checkout`);
  };
    ...
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        ...
        <AppleCard className="mb-8">
          <h2 className="text-2xl font-semibold text-gray-800 mb-6">Add-ons para Trippers Decode</h2>
          <form onSubmit={handleSubmit} className="space-y-6">
            {decodeAddons.map((addon) => (
              <AddonOption
                key={addon.id}
                id={addon.id}
                title={addon.title}
...
export default DecodeAddonsPage;
