...
// Transitions a PENDING_REVIEW experience back to DRAFT, saves reviewNote.
  ...
  try {
    ...
    if (experience.status !== "PENDING_REVIEW") {
      return NextResponse.json(
        {
          error: "invalid_state",
          message: "Experience must be in PENDING_REVIEW status to reject",
        },
        { status: 409 },
      );
    }

    const body = await request.json() as { reviewNote?: unknown };
    const reviewNote =
    ...
    console.error("[admin/experiences/reject] POST", error);
    return NextResponse.json(
