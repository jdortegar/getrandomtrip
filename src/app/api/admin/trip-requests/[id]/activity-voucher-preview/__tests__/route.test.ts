...
const document = {
  ...
  data: {
    ...
    date: "2026-09-23",
    time: "10:00",
    provider: { name: "Río Spa", address: "Calle 12" },
    program: [{ id: "spa", title: "Circuito de agua" }],
    inclusions: [],
  },
};
...
beforeEach(() => {
  ...
  vi.mocked(renderActivityVoucher).mockResolvedValue({
    ok: true,
    buffer: Buffer.from("%PDF-test"),
  });
});
describe("activity preview POST", () => {
  it("requires authentication before reading the body", async () => {
    ...
    expect(renderActivityVoucher).not.toHaveBeenCalled();
  ...
  it.each(["en", "es"])(
    ...
    async (locale) => {
      ...
      expect(response.headers.get("content-disposition")).toBe(
        'inline; filename="activity-voucher-preview.pdf"',
      );
      expect(renderActivityVoucher).toHaveBeenCalledWith({
        ...document,
        locale,
      });
    },
  ...
  it.each([
    { ...document, locale: "fr" },
    { ...document, extra: true },
    { ...document, template: "hotel-voucher" },
    { ...document, data: { ...document.data, program: [] } },
    { ...document, data: { ...document.data, time: "25:00" } },
    { ...document, data: { ...document.data, date: "2026-02-30" } },
  ...
  it.each([
    ...
    vi.mocked(renderActivityVoucher).mockRejectedValue(
