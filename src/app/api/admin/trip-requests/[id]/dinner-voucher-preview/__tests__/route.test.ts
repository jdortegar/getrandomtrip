...
vi.mock("@/lib/trip-documents/pdf/renderDinnerVoucher", () => ({
  renderDinnerVoucher: vi.fn(),
}));
...
const document = {
  ...
  data: {
    ...
    date: "2026-09-23",
    time: "10:00",
    restaurant: { name: "Río Spa", address: "Calle 12" },
    service: "Cena de pasos",
    menuItems: [{ id: "starter", title: "Entrada" }],
  },
};
...
beforeEach(() => {
  ...
  vi.mocked(renderDinnerVoucher).mockResolvedValue({
    ok: true,
    buffer: Buffer.from("%PDF-test"),
  });
});
describe("dinner preview POST", () => {
  it("requires authentication before reading the body", async () => {
    ...
    expect(renderDinnerVoucher).not.toHaveBeenCalled();
  ...
  it.each(["en", "es"])(
    ...
    async (locale) => {
      ...
      expect(response.headers.get("content-disposition")).toBe(
        'inline; filename="dinner-voucher-preview.pdf"',
      );
      expect(renderDinnerVoucher).toHaveBeenCalledWith({
        ...document,
        locale,
      });
    },
  ...
  it.each([
    ...
    vi.mocked(renderDinnerVoucher).mockRejectedValue(
