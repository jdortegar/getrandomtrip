...
vi.mock("@/lib/trip-documents/pdf/renderExperienceRoadmap", () => ({
  renderExperienceRoadmap: vi.fn(),
}));
import { getServerSession } from "next-auth";
import { prisma } from "@/lib/prisma";
import { renderExperienceRoadmap } from "@/lib/trip-documents/pdf/renderExperienceRoadmap";
...
beforeEach(() => {
  ...
  vi.mocked(renderExperienceRoadmap).mockResolvedValue({
    ok: true,
    buffer: Buffer.from("%PDF-test"),
  });
});
describe("experience roadmap preview POST", () => {
  it("requires authentication before reading the body", async () => {
    ...
    expect(renderExperienceRoadmap).not.toHaveBeenCalled();
  ...
  it.each(["en", "es"])(
    ...
    async (locale) => {
      ...
      expect(response.headers.get("content-disposition")).toBe(
        'inline; filename="experience-roadmap-preview.pdf"',
      );
      expect(renderExperienceRoadmap).toHaveBeenCalledWith({
        ...document,
        locale,
      });
    },
  ...
  it.each([
    ...
    { ...document, data: { ...document.data, startDate: "2026-02-30" } },
  ...
  it.each([
    ...
    vi.mocked(renderExperienceRoadmap).mockRejectedValue(
