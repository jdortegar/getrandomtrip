...
vi.mock("@/lib/trip-documents/pdf/renderXsedRoadmap", () => ({
  renderXsedRoadmap: vi.fn(),
}));
import { getServerSession } from "next-auth";
import { prisma } from "@/lib/prisma";
import { renderXsedRoadmap } from "@/lib/trip-documents/pdf/renderXsedRoadmap";
...
const document = {
  ...
  data: {
    ...
    departureTime: "09:00",
    stops: [
      { id: "walk", title: "Paseo", directions: "Caminar por el río" },
    ],
  },
};
...
beforeEach(() => {
  ...
  vi.mocked(renderXsedRoadmap).mockResolvedValue({
    ok: true,
    buffer: Buffer.from("%PDF-test"),
  });
});
describe("XSED roadmap preview POST", () => {
  it("requires authentication before reading the body", async () => {
    ...
    expect(renderXsedRoadmap).not.toHaveBeenCalled();
  ...
  it.each(["en", "es"])(
    ...
    async (locale) => {
      ...
      expect(response.headers.get("content-disposition")).toBe(
        'inline; filename="xsed-roadmap-preview.pdf"',
      );
      expect(renderXsedRoadmap).toHaveBeenCalledWith({
        ...document,
        locale,
      });
    },
  ...
  it.each([
    ...
    { ...document, data: { ...document.data, departureDate: "2026-02-30" } },
  ...
  it.each([
    ...
    vi.mocked(renderXsedRoadmap).mockRejectedValue(
