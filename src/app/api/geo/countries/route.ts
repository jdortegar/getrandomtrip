...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error('[geo/countries] Mapbox error:', res.status, res.statusText);
      return NextResponse.json({ results: [] });
    }

    const data = (await res.json()) as { features: { text: string; place_name: string; properties: { short_code?: string } }[] };
