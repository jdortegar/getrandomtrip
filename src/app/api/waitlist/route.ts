...
function validateBody(
  body: unknown,
): { email: string; name: string | null; lastName: string | null } | null {
  if (
    !body ||
    typeof body !== "object" ||
    !("email" in body) ||
  ...
  const { email, name, lastName } = body as {
    email: string;
    name?: unknown;
    lastName?: unknown;
  };
  ...
  if (normalizedName !== null && normalizedName.length > MAX_NAME_LENGTH)
    return null;
  if (
    normalizedLastName !== null &&
    normalizedLastName.length > MAX_NAME_LENGTH
  )
    return null;
  return {
    email: trimmedEmail,
