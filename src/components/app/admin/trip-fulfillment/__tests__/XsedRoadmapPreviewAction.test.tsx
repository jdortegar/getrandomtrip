...
import { XsedRoadmapPreviewAction } from "../XsedRoadmapPreviewAction";
(
...
beforeEach(() => {
  ...
  vi.stubGlobal(
    "URL",
    class extends URL {
      static createObjectURL = vi.fn(() => "blob:xsed");
      static revokeObjectURL = revoke;
    },
  );
  act(() =>
    root.render(
      <XsedRoadmapPreviewAction
...
it("opens a transient form with editable known trip snapshots, never provider claims", () => {
  act(() => button(en.xsedRoadmapPreview.open).click());
  expect(document.querySelector('[role="dialog"]')?.textContent).toContain(
    en.xsedRoadmapPreview.note,
  );
  expect(
    (document.getElementById("xsed-origin") as HTMLInputElement).value,
  ).toBe("Rosario");
  expect(
    (document.getElementById("xsed-locale") as HTMLSelectElement).value,
  ).toBe("es");
  expect(
    (document.getElementById("xsed-departureDate") as HTMLInputElement).value,
  ).toBe("2026-10-01");
  expect(
    (document.getElementById("xsed-destination") as HTMLInputElement)
      .value,
  ).toBe("");
  expect(
    (document.getElementById("xsed-departureTime") as HTMLInputElement).value,
  ).toBe("");
});
it("previews the edited form, invalidates on edit, confirms discard and resets on reopening", async () => {
  const confirm = vi.spyOn(window, "confirm").mockReturnValue(false);
  act(() => button(en.xsedRoadmapPreview.open).click());
  edit("xsed-country", "AR");
  edit("xsed-destination", "Areco");
  edit("xsed-departureTime", "09:00");
  fillStops();
  ...
  expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toMatchObject({
    locale: "es",
    data: { destination: "Areco", departureTime: "09:00" },
  });
  expect(document.querySelector("iframe")?.getAttribute("src")).toBe(
    "blob:xsed",
  );
  edit("xsed-destination", "Edited");
  expect(document.querySelector("iframe")).toBe(null);
  expect(revoke).toHaveBeenCalledWith("blob:xsed");
  act(() => button(en.hotelVoucherPreview.close).click());
  expect(confirm).toHaveBeenCalledWith(en.xsedRoadmapPreview.discard);
...
it("shows localized request errors without leaking server details", async () => {
  act(() => button(en.xsedRoadmapPreview.open).click());
  edit("xsed-country", "AR");
  edit("xsed-destination", "Ana");
  edit("xsed-destination", "Hotel");
  edit("xsed-departureTime", "09:00");
  fillStops();
...
function fillStops() {
  edit("xsed-drivingDuration", "2 horas");
  act(() =>
