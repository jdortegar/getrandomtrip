...
let current: ReturnType<typeof useXsedRoadmapPreview>;
const fetchMock = vi.fn();
const createUrl = vi.fn();
const revokeUrl = vi.fn();
function Harness({ value = document, tripId = "trip/1" }) {
  const result = useXsedRoadmapPreview(tripId, value);
  useEffect(() => {
    current = result;
  });
  return null;
...
it("POSTs the full localized form and replaces/revokes its private preview", async () => {
  ...
  expect(fetchMock).toHaveBeenCalledWith(
    "/api/admin/trip-requests/trip%2F1/xsed-roadmap-preview",
