...
function ChipInput({
  ...
  return (
    <div className="flex flex-col gap-2">
      <label className="block font-normal text-gray-600 text-base" htmlFor={id}>{label}</label>
      <input
        id={id}
      ...
      {values.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-1">
          {values.map((v, i) => (
            <span key={i} className="flex items-center gap-1.5 bg-neutral-100 border border-neutral-200 rounded-full px-3 py-1 text-sm text-neutral-700">
              {v}
              <button type="button" onClick={() => onRemove(i)} className="text-neutral-400 hover:text-red-500 transition-colors">
                <X className="h-3 w-3" />
              </button>
            </span>
          ))}
...
function HeroImageUpload({
  ...
  return (
    <div className="flex flex-col gap-2">
      <label className="block font-normal text-gray-600 text-base">{label}</label>

      {value ? (
        <div className="relative w-full h-52 rounded-xl overflow-hidden border border-gray-200 group">
          <Img src={value} alt="Hero" width={900} height={208} className="object-cover w-full h-full" unoptimized />
          <button
            type="button"
            onClick={onRemove}
  ...
  return (
    <div className="space-y-5">
      ...
      <ChipInput
        ...
        onRemove={(i) => onChange('tags', form.tags.filter((_, idx) => idx !== i))}
      />
      ...
      <ChipInput
        ...
        onRemove={(i) => onChange('highlights', form.highlights.filter((_, idx) => idx !== i))}
      />
    </div>
  );
}
