    ...
    >
      <div className="rt-container grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
        <div className="md:col-span-1 text-left">
          ...
          >
            Inspiración del Tripper
          </h2>
          <p className="text-lg text-gray-300 mt-4">
            Notas, guías y momentos que inspiran.
          </p>
        </div>
        <div className="md:col-span-2">
          <div className="flex overflow-x-auto space-x-8 pb-8 hide-scrollbar">
            {items.map(
              (p: { title: string; image: string; category: string }) => (
                <div
                  key={p.title}
                  className="w-80 shrink-0 transition hover:-translate-y-0.5 hover:shadow-lg"
                >
