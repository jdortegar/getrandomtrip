...
export default function TripperVisitedMap({
  places = [] as VisitedPlace[],
}: {
  places?: VisitedPlace[];
}) {
  ...
  const initialCenter: [number, number] = hasPlaces
    ? [places[0].lat, places[0].lng]
    : [0, 0];

  return (
    <section className="bg-primary text-white rt-container px-4 md:px-8 py-12" data-component="TripperVisitedMap">
      {!hasPlaces ? (
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    m.flyTo(
                      [p.lat, p.lng],
                      Math.max(m.getZoom(), defaultZoom),
                      { duration: 0.8 },
                    );
