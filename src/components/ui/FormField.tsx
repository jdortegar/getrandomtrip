    ...
    return (
      <div className="flex flex-col gap-2">
        ...
        <div className="relative">
          <input
            className={cn(formControlClass, isPasswordField && 'pr-12', className)}
            id={id}
            ref={ref}
            type={resolvedType}
            {...inputProps}
...
export interface FormSelectFieldProps extends Omit<React.ComponentProps<'select'>, 'id'> {
  children: ReactNode;
  id: string;
  label: ReactNode;
}
...
export const FormSelectField = React.forwardRef<HTMLSelectElement, FormSelectFieldProps>(
  function FormSelectField({ children, className, id, label, ...selectProps }, ref) {
    return (
      <div className="flex flex-col gap-2">
        <label className={formLabelClass} htmlFor={id}>
          {label}
