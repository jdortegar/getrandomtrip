...
export function getMissingFields(
  ...
  switch (tabId) {
    ...
    case 'capacity':
      ...
      if (!form.displayPrice) m.push(labels.displayPrice ?? 'Precio de visualización');
      break;
    case 'logistics':
      if (!form.travelTime) m.push(labels.estimatedTravelTime ?? 'Tiempo estimado de traslado');
      if (!form.accommodations[0]?.hotelName) m.push(labels.hotelName ?? 'Nombre del hotel');
      if (!form.accommodations[0]?.hotelLocation) m.push(labels.hotelLocation ?? 'Ubicación del hotel');
      break;
    case 'activities':
      if (!form.activities[0]?.name) m.push(labels.activityName ?? 'Nombre de la actividad');
      if (!form.itinerary[0]?.title) m.push(labels.itineraryTitle ?? 'Titulo del día 1');
      break;
  }
  return m;
}
